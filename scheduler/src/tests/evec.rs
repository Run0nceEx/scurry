...
use super::evec_mock::{JOB_CNT, POOLSIZE};

use tokio::runtime::Runtime;

#[test]
fn single_in_single_out() {
    use super::evec_mock::noop;
...
#[test]
fn all_in_all_out() {
    use super::evec_mock::noop::get_pool;
    let mut rt = Runtime::new().unwrap();
...
fn all_retry_now_once() {
    use super::evec_mock::noop as evec_mock;
    ...
    rt.block_on(async move {
        let mut pool = evec_mock::get_pool(100.0, 0.0, 3);
...
fn all_timeout() {
    ...
    impl CRON for Worker {
        type State = evec_mock::State;
        type Response = evec_mock::Response;

        async fn exec(_state: &mut Self::State) -> Result<(SignalControl, Option<Self::Response>), Error> {            
            tokio::time::delay_for(Duration::from_secs(3)).await;
            Ok((SignalControl::Success(false), Some(evec_mock::Response)))
        }
    ...
    rt.block_on(async move {
        let mut pool: CronPool<Worker, evec_mock::Response, evec_mock::State> = CronPool::new(POOLSIZE);

        let live_for = Duration::from_secs(1);

        for _ in 0..JOB_CNT {
            pool.insert(evec_mock::State, live_for, Duration::from_secs(0), 1);
        }
